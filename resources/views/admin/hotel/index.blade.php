...
<div class="toolbar" id="kt_toolbar">
    ...
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        ...
        <div class="d-flex align-items-center me-3">
            <!--begin::Title-->
            <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">{{__('Hotels')}}
            <!--begin::Separator-->
...
<div class="card rounded mb-5 mb-xl-8 shadow-lg">
    ...
    <div class="card-header rounded border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">{{__('Hotels')}}</span>
        </h3>
        <div class="card-toolbar">
            <a href="{{route('hotel.create')}}" class="btn btn-sm btn-light-primary">
            ...
            <table class="table table-hover align-middle gs-0 gy-4">
                ...
                <thead>
                    <tr class="text-center border-3 fw-bolder text-muted bg-light">
                        <th class="ps-4 min-w-325px rounded-start">{{__('Hotel')}}</th>
                        <th class="ps-4 min-w-325px rounded-start">{{__('Content')}}</th>
                        <th class="min-w-200px">{{__('Vendor')}}</th>
                        <th class="min-w-200px">{{__('City')}}</th>
                        <th class="min-w-200px">{{__('Star Rate')}}</th>
                        <th class="min-w-200px">{{__('Home')}}</th>
                        <th class="min-w-200px">{{__('order')}}</th>
                        ...
                        <td class="px-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="d-flex flex-column">
                                    <a href="{{ route('hotel.edit', $p->id) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{$p->title}}</a>
                                    <!-- <span class="text-muted fw-bold text-muted d-block fs-7">{{$p->title}}</span> -->
                                </div>
                            </div>
                        </td>
                        <td class="px-3">
                         ...
                         <td class="px-3">
                            <a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{$p->vendor->name  ?? ''}}</a>
                        </td>
                         <td class="px-3">
                            <a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{$p->gov->name_en ?? ' ' }}</a>
                        </td>
                         <td class="px-3">
                            <a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{$p->star_rate}}</a>
                        </td>
                        <td class="px-3">
                            <div class="d-flex flex-column">
                                @if($p->home == 1)
                         ...
                         <td class="px-3">
                            <span class="badge badge-light-primary fw-bolder px-4 py-3">{{$p->order_no}}</span>
                        </td>
                         ...
                         <td class="border text-center">
                            ...
                            <a href="{{ route('hotel.edit', $p->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                            ...
                            <form action="{{ route('hotel.destroy', $p->id) }}" method="post" id='delform' style="display: inline-block">
                                @csrf
                                @method('delete')
